...
observe({
  ...
  output$latestreading <- renderText(paste0('<a href="',filtmonsites$resultsetnoangle,'?tab=api" target="_blank">',formatNos(filtmonsites$value),' m<sup>3</sup> / sec</a>'))
  output$latestdatetime <- renderText(paste0('<a href="',filtmonsites$resultsetnoangle,'?tab=api" target="_blank">',format(as.POSIXct(filtmonsites$latest, origin='1970-01-01'),"%Y-%m-%d %X"),'</a>'))
  output$maxflow <- renderText(paste0(ifelse(is.na(filtmonsites$maxflowval),'Not available',paste0('<a href="',filtmonsites$maxflownoangle,'?tab=api" target="_blank">',formatNos(filtmonsites$maxflowval),' m<sup>3</sup> / sec</a>'))))
  output$meanflow <- renderText(paste0(ifelse(is.na(filtmonsites$meanannflowval),'Not available',paste0('<a href="',filtmonsites$meanannflownoangle,'?tab=api" target="_blank">',formatNos(filtmonsites$meanannflowval),' m<sup>3</sup> / sec</a>'))))
  output$malf <- renderText(paste0(ifelse(is.na(filtmonsites$malfval),'Not available',paste0('<a href="',filtmonsites$malfnoangle,'?tab=api" target="_blank">',formatNos(filtmonsites$malfval),' m<sup>3</sup> / sec</a>'))))
  output$minflow <- renderText(paste0(ifelse(is.na(filtmonsites$minflowval),'Not available',paste0('<a href="',filtmonsites$minflownoangle,'?tab=api" target="_blank">',formatNos(filtmonsites$minflowval),' m<sup>3</sup> / sec</a>'))))
  output$meanfloodflow <- renderText(paste0(ifelse(is.na(filtmonsites$meanfloodflowval),'Not available',paste0('<a href="',filtmonsites$meanfloodflownoangle,'?tab=api" target="_blank">',formatNos(filtmonsites$meanfloodflowval),' m<sup>3</sup> / sec</a>'))))
  output$climate <-renderText(paste0(ifelse(is.na(filtmonsites$climatelabel),'Not available',paste0('<a href="',filtmonsites$climatenoangle,'?tab=api" target="_blank">',filtmonsites$climatelabel,'</a>'))))
  output$elevation <-renderText(paste0(ifelse(is.na(filtmonsites$elevation),'Not available',paste0('<a href="http://envdatapoc.co.nz/doc/measurement-site/',filtmonsites$siteID,'?tab=api" target="_blank">',filtmonsites$elevation,'m</a>'))))
  output$geology <-renderText(paste0(ifelse(is.na(filtmonsites$geologylabel),'Not available',paste0('<a href="',filtmonsites$geologynoangle,'?tab=api" target="_blank">',filtmonsites$geologylabel,'</a>'))))
  output$landcover <-renderText(paste0(ifelse(is.na(filtmonsites$landcover),'Not available',paste0('<a href="',filtmonsites$landcovernoangle,'?tab=api" target="_blank">',filtmonsites$landcoverlabel,'</a>'))))
  output$reachId <-renderText(paste0(ifelse(is.na(filtmonsites$reach),'Not available',paste0('<a href="',filtmonsites$reachnoangle,'?tab=api" target="_blank">',filtmonsites$reachlabel,'</a>'))))
